# ─────────────────────────────────────────────────────────────────────────────
# Czech Data Protection MCP — multi-stage Dockerfile
# ─────────────────────────────────────────────────────────────────────────────
# Build:  docker build -t czech-data-protection-mcp .
# Run:    docker run --rm -p 3000:3000 czech-data-protection-mcp
#
# The image bakes /app/data/uoou.db at build time. Override UOOU_DB_PATH
...
ENV UOOU_DB_PATH=/app/data/uoou.db
...
# Bake the pre-built database into the image so /app/data/uoou.db resolves
...
# We then COPY it into the image at /app/data/uoou.db (UOOU_DB_PATH). The
# explicit `data/<name>.db` reference is required for the workflow's grep
# `COPY\s+\K(data/\S+\.db)` to match — a directory-form `COPY data/` would
# be skipped by that regex and the DB would never reach the image.
COPY data/database.db data/uoou.db
